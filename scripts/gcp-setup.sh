...
create_credentials() {
    ...
    info "     (Desktop type uses loopback redirect — no URIs to configure.)"
    info "     (Google treats Desktop credentials as non-confidential.)"
...
# ─── Step 6: Write Secrets.xcconfig ────────────────────────────────────────────

configure_environment() {
    step_header "6" "Write Build Credentials"

    local xcconfig_file="$PROJECT_DIR/Secrets.xcconfig"

    info "Writing credentials to ${BOLD}Secrets.xcconfig${RESET}..."
    info "This file is gitignored and injected into the app bundle at build time."
    echo ""
    info "Google treats Desktop app credentials as ${BOLD}non-confidential${RESET}."
    info "The client_secret is safe to embed in the built application."
    info "Ref: https://developers.google.com/identity/protocols/oauth2/native-app"
    echo ""

    # Back up existing file if present
    if [[ -f "$xcconfig_file" ]]; then
        warn "Secrets.xcconfig already exists."
        if ! confirm_yes_no "Overwrite with new credentials?"; then
            info "Skipped. Existing credentials preserved."
            return
        fi
    fi
...
// MaxBox OAuth2 Credentials (Desktop app — non-confidential per Google policy)
// Generated by gcp-setup.sh on $(date +%Y-%m-%d)
// GCP Project: ${GCP_PROJECT_ID}
//
// DO NOT check this file into version control.

MAXBOX_GMAIL_CLIENT_ID = ${CLIENT_ID}
MAXBOX_GMAIL_CLIENT_SECRET = ${CLIENT_SECRET}
EOF

    chmod 600 "$xcconfig_file"
    success "Written to ${BOLD}Secrets.xcconfig${RESET} (permissions: 600)"

    # Verify .gitignore has the entry
    ensure_gitignore

    echo ""
    info "Credentials are baked into the app at build time via Info.plist."
    info "No environment variables or shell sourcing needed."
}

ensure_gitignore() {
    local gitignore="$PROJECT_DIR/.gitignore"
    local entry="Secrets.xcconfig"
...
print_summary() {
    ...
    echo "    ${DIM}2.${RESET} Run MaxBox:          ${DIM}./scripts/run.sh${RESET}"
    echo "    ${DIM}3.${RESET} Add your account:    ${DIM}Click \"Add Account\" in the sidebar${RESET}"
    echo ""
    echo "${DIM}  Credentials are embedded at build time. No env vars needed.${RESET}"
